...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  const allowedFromats = ["image/jpg", "image/webp", "image/png"];
  if (allowedFromats.includes(image.mimetype)) {
  ...
  if (alreadyOneAuctionActive.length > 0) {
    return next(new ErrorHandler("you Alerady have one active auction.", 400));
  }
  ...
  try {
    ...
    if (!cloudinaryResponse || cloudinaryResponse.error) {
      ...
      return next(
        new ErrorHandler("Failed to uplaod auction image to cloudinary", 500)
      );
    }
...
export const republishedItem = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return next(new ErrorHandler("Invalid Id format.", 400));
  }
  ...
  if (new Date(auctionItem.endTime) > Date.now()) {
    return next(
      new ErrorHandler("Auction is already active, cannot republished.", 400)
    );
  }
  let data = {
